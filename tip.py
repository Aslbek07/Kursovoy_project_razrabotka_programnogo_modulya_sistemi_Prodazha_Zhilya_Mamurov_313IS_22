...
# Создается новый класс ChangeTipWindow который наследуется от класса QMainWindow. 
class ChangeTipWindow(QMainWindow):
    ...
    def read_record(self):
        try:
            ...
            sql = "SELECT id, nazvanie FROM tip WHERE id=" + str(self.record_id)
    ...
    def update_data(self):
        try:
            ...
            if self.record_id==0:
                sql = "INSERT INTO tip (nazvanie) VALUES (?)"
            else:
                sql = "UPDATE tip SET nazvanie=? WHERE id=?"
            #print(sql)
            # Параметры запроса
            parameters = [nazvanie]
            if self.record_id != 0:
...
# Создается новый класс TipListWindow котрый наследуется от класса QMainWindow. 
class TipListWindow(QMainWindow):
    def __init__(self):
        try:
            super().__init__()
            # Метод setGeometry() помещает окно на экране и устанавливает его размер.
    ...
    def insert_data(self):
        try:
            ...
            self.change_tip_window = ChangeTipWindow(0)        
            self.change_tip_window.setWindowTitle(msg.add)
            self.change_tip_window.show()
        except Exception as error:
            print(error)
            QMessageBox.critical(self,  "Error",  str(error))   
    ...
    def edit_data(self):
        try:
            ...
            _id = self.tableWidget.item(self.tableWidget.currentRow(), 0).text()
            # Вызов дочернего окна для изменения записи
            self.change_tip_window = ChangeTipWindow(_id)     
            self.change_tip_window.setWindowTitle(msg.change)
            self.change_tip_window.show()   
        except Exception as error:
            print(error)
            QMessageBox.critical(self,  "Error",  str(error))   
