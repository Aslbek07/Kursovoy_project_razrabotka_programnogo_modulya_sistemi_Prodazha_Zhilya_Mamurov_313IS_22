...
# Создается новый класс ChangeDolzhnostWindow который наследуется от класса QMainWindow. 
class ChangeDolzhnostWindow(QMainWindow):
    def __init__(self, _id):
        try:
            ...
            # Метки
            self.label_nazvanie = QLabel(msg.title)
            # Поля ввода данных
            self.lineEdit_nazvanie = QLineEdit(self)
            # Создаём центральный виджет
            central_widget = QWidget(self)
            central_widget.setMaximumHeight(100)
            ...
            grid_layout.addWidget(self.label_nazvanie, 0, 0, 1, 1)
            grid_layout.addWidget(self.lineEdit_nazvanie, 0, 1, 1, 3) 
            grid_layout.addWidget(self.pushButtonUpdate, 1, 0, 1, 2) 
            grid_layout.addWidget(self.pushButtonCancel, 1, 2, 1, 2)             
    ...
    def read_record(self):
        try:
            ...
            sql = "SELECT id, nazvanie FROM dolzhnost WHERE id=" + str(self.record_id)
            ...
            self.lineEdit_nazvanie.setText(results[1])
        except Exception as error:
            print(error)
            QMessageBox.critical(self,  "Error",  str(error))   
    ...
    def update_data(self):
        try:
            # Данные
            nazvanie = self.lineEdit_nazvanie.text()
            # Проверка данных
            if nazvanie == "" :
            ...
            if self.record_id==0:
                sql = "INSERT INTO dolzhnost (nazvanie) VALUES (?)"
            else:
                sql = "UPDATE dolzhnost SET nazvanie=? WHERE id=?"
            #print(sql)
            # Параметры запроса
            parameters = [nazvanie]
            if self.record_id != 0:
...
# Создается новый класс DolzhnostListWindow котрый наследуется от класса QMainWindow. 
class DolzhnostListWindow(QMainWindow):
    def __init__(self):
        try:
            super().__init__()
            # Метод setGeometry() помещает окно на экране и устанавливает его размер.
            ...
            self.setGeometry(200, 200, 800, 600)
            # Заголовок окна
            self.setWindowTitle(msg.dolzhnosti)
            #self.setWindowIcon(QtGui.QIcon('logo.png'))
            self.setWindowIcon(QtGui.QIcon(':/images/logo.png'))
            # Создаём таблицу
            ...
            self.tableWidget.setHorizontalHeaderLabels([msg.id, msg.title])
    ...
    def select_data(self, where):
        try:
            # SQL-запрос
            sql = "SELECT * FROM dolzhnost " + where
            # Получить результат запроса из resultSet можно с помощью методов, например, fetchall() или fetchone()
    ...
    def filter_data(self):
        try:
            ...
            where = " WHERE nazvanie Like '%" +  self.lineEdit.text() + "%'"
    ...
    def delete_data(self):
        try:
            ...
            if reply == QMessageBox.Yes:
                ...
                sql = "DELETE FROM dolzhnost WHERE id=?"
                # Параметры запроса
                parameters = [_id]
                # Выполнить запрос sql c параметрами parameters
                database.executeSQL(sql, parameters)
    ...
    def insert_data(self):
        try:
            ...
            self.change_dolzhnost_window = ChangeDolzhnostWindow(0)        
            self.change_dolzhnost_window.setWindowTitle(msg.add)
            self.change_dolzhnost_window.show()
        except Exception as error:
            print(error)
            QMessageBox.critical(self,  "Error",  str(error))   
    ...
    def edit_data(self):
        try:
            ...
            _id = self.tableWidget.item(self.tableWidget.currentRow(), 0).text()
            # Вызов дочернего окна для изменения записи
            self.change_dolzhnost_window = ChangeDolzhnostWindow(_id)     
            self.change_dolzhnost_window.setWindowTitle(msg.change)
            self.change_dolzhnost_window.show()   
        except Exception as error:
            print(error)
            QMessageBox.critical(self,  "Error",  str(error))   
