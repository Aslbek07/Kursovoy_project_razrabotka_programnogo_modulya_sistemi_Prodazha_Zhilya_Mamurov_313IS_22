...
# Создается новый класс ChangeKlientWindow который наследуется от класса QMainWindow. 
class ChangeKlientWindow(QMainWindow):
    def __init__(self, _id):
        try:
            ...
            grid_layout.addWidget(self.pushButtonUpdate, 3, 0, 1, 3) 
            grid_layout.addWidget(self.pushButtonCancel, 3, 3, 1, 3)  
    ...
    def read_record(self):
        try:
            ...
            sql = "SELECT id, fio, adres, telefon FROM klient WHERE id=" + str(self.record_id)
    ...
    def update_data(self):
        try:
            ...
            if self.record_id==0:
                ...
                sql = "UPDATE klient SET fio=?, adres=?, telefon=? WHERE id=?"
            #print(sql)
            # Параметры запроса
            parameters = [fio, adres, telefon]
            if self.record_id != 0: 
...
# Создается новый класс KlientListWindow котрый наследуется от класса QMainWindow. 
class KlientListWindow(QMainWindow):
    def __init__(self):
        try:
            super().__init__()
            # Метод setGeometry() помещает окно на экране и устанавливает его размер.
            ...
            self.setGeometry(200, 200, 800, 600)
            # Заголовок окна
            self.setWindowTitle(msg.klienty)
            #self.setWindowIcon(QtGui.QIcon('logo.png'))
            self.setWindowIcon(QtGui.QIcon(':/images/logo.png'))
            # Создаём таблицу
            ...
            self.tableWidget.setHorizontalHeaderLabels([msg.id, msg.fio, msg.adres, msg.telefon])
    ...
    def select_data(self, where):
        try:
            ## SQL-запрос
            sql = "SELECT id, fio, adres, telefon FROM klient " + where
            # Получить результат запроса из resultSet можно с помощью методов, например, fetchall() или fetchone()
    ...
    def delete_data(self):
        try:
            ...
            if reply == QMessageBox.Yes:
                ...
                sql = "DELETE FROM klient WHERE id=?"
                # Параметры запроса
                parameters = [_id]
                # Выполнить запрос sql c параметрами parameters
                database.executeSQL(sql, parameters)
    ...
    def insert_data(self):
        try:
            ...
            self.change_klient_window = ChangeKlientWindow(0)    
            self.change_klient_window.setWindowTitle(msg.add)
            self.change_klient_window.show()
        except Exception as error:
            print(error)
            QMessageBox.critical(self,  "Error",  str(error))   
    ...
    def edit_data(self):
        try:
            ...
            _id = self.tableWidget.item(self.tableWidget.currentRow(), 0).text()
            # Вызов дочернего окна для изменения записи
            self.change_klient_window = ChangeKlientWindow(_id)   
            self.change_klient_window.setWindowTitle(msg.change)
            self.change_klient_window.show()   
        except Exception as error:
            print(error)
            QMessageBox.critical(self,  "Error",  str(error))   
